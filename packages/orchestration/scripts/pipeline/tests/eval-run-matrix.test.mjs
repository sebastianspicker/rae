...
function matrixFailureFixture(baseDir) {
  ...
  return { fixtureRoot, pipelineDir, sentinelState, sentinelRun };
}

function writeFailingTaskset(defaultTaskset, tasksetPath, unique) {
...
function expectFailedMatrixRuns(matrix, runsDir) {
...
describe("run-matrix end-to-end", { timeout: 60_000 }, () => {
  it("closes each abandoned failed run without touching another pipeline root", () => {
    ...
    const { fixtureRoot, pipelineDir, sentinelState, sentinelRun } = matrixFailureFixture(baseDir);
    ...
    try {
      ...
      writeFailingTaskset(defaultTaskset, tasksetPath, unique);
      ...
      expectFailedMatrixRuns(matrix, runsDir);
