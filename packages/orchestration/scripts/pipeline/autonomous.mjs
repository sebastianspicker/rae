...
async function main() {
  ...
  if (["status", "stop", "signal", "resolve-checkpoint", "events"].includes(command)) {
    const { runControlCommand } = await import("./lib/autonomous-actions.mjs");
    return runControlCommand(command, options);
  }
  if (["run", "resume"].includes(command)) {
    const { runWorkflow } = await import("./lib/autonomous-actions.mjs");
...
async function runDoctor(options) {
  const [{ agentDoctor }, { loadExecutionProfile }] = await Promise.all([
    import("./lib/agent-executor.mjs"),
    import("./lib/execution-profile.mjs"),
  ]);
  const baseOptions = {
