...
function buildExecuted(gates) {
  return gates.some((gate) => gate.phase === "build" && gate.status !== "not-run");
}
function eventLogs(workspaceRoot, runDir) {
...
function cleanupLine(cleanupCommand) {
  return `- Cleanup command: ${cleanupCommand ? `\`${cleanupCommand}\`` : "not applicable"}`;
}
function workspaceLine(context) {
  return `- Workspace: \`${context.workspaceRoot}\``;
}
function workspaceModeLine(state) {
  return `- Workspace mode: \`${state.workspace?.mode ?? "unknown"}\``;
}
function branchLine(state) {
  return `- Branch: \`${state.workspace?.branch || "(detached or unchanged)"}\``;
}
function providerLine(outcome) {
  return `- Provider: \`${outcome.provider ?? "unknown"}\``;
}
function policyLine(context) {
...
function residualSection(outcome) {
  return [
    "## Residual state",
    "",
    ...(outcome.error
...
function nextAction(workspaceRoot, cleanupCommand) {
  return [
    "## Next action",
    "",
