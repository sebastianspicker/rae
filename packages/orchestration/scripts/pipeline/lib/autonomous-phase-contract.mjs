...
function validPlanPathText(path) {
  return typeof path === "string" && Boolean(path) && !path.includes("\0");
}
function invalidWildcard(path) {
  return path.includes("*") && !path.endsWith("/**");
}
function validPlanSegments(segments) {
  return !segments.some(invalidPlanSegment) && ![".git", ".pipeline"].includes(segments[0]);
}
function invalidPlanSegment(part) {
...
export function gateStatusForArtifact(phase, artifact) {
  ...
  return artifact.release_decision === "no-go"
    ? "fail"
...
function phaseGateEvaluator(phase) {
  return {
    arm: armGate,
    "adversarial-review": reviewGate,
    pmatch: pmatchGate,
    plan: planGate,
    "quality-static": qualityGate,
    "quality-tests": qualityGate,
    "post-build": qualityGate,
  }[phase];
}
function armGate(artifact) {
...
function missingQualityEvidence(evidence) {
  return (
    !evidence ||
...
function failedQualitySummary(summary = {}) {
  return (summary.fail ?? 0) > 0 || (summary.open ?? 0) > 0;
}
function warnedQualitySummary(evidence, summary = {}) {
  return (
