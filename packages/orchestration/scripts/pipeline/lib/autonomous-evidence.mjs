...
function recordMissingEvidence(phase, artifact) {
  if (phase === "build") {
    artifact.groups = [
      ...(artifact.groups ?? []),
      { group_id: "runtime-command-evidence", status: "fail", tasks_completed: 0, tasks_total: 1, errors: ["Codex emitted no command_execution event for build verification"] },
    ];
    return;
  }
  artifact.violations = [...(artifact.violations ?? []), { rule: "command-execution-evidence", severity: "high", file: ".pipeline", evidence: `Codex emitted no command_execution event during ${phase}`, remediation: "Execute the required project verification and return its evidence", status: "open", ...(phase === "post-build" ? { category: "production-exposure" } : {}) }];
  artifact.summary = { ...(artifact.summary ?? {}), fail: Math.max(1, artifact.summary?.fail ?? 0), open: Math.max(1, artifact.summary?.open ?? 0) };
  artifact.evidence_bundle = {
    status: "partial",
    references: artifact.evidence_bundle?.references ?? [],
    missing_types: [...new Set([...(artifact.evidence_bundle?.missing_types ?? []), "codex-command-event"])],
    residual_gaps: [...new Set([...(artifact.evidence_bundle?.residual_gaps ?? []), `No successful completed plan.verification_commands execution was captured for ${phase}`])],
  };
}
...
export function enforceCommandEvidence(phase, result, artifact, plan, workspaceRoot) {
  if (result.provider !== "codex" || !EVIDENCE_PHASES.has(phase)) return { required: result.provider === "codex", status: "not-applicable" };
  ...
  const matched = required.filter((entry) => {
    ...
    return workingDirectory !== null && keys.has(`${workingDirectory}\0${normalizeCommand(entry.command)}`);
  });
  if (required.length > 0 && matched.length === required.length) {
    return { required: true, status: "present", command_event_count: result.commandEventCount, successful_command_event_count: keys.size, matched_planned_command_count: matched.length, required_planned_command_count: required.length };
  }
