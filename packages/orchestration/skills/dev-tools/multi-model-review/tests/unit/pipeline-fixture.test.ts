...
function loadSchema(pathFromRepoRoot: string): Record<string, unknown> {
  return JSON.parse(readFileSync(resolve(repoRoot, pathFromRepoRoot), "utf8")) as Record<
    string,
    unknown
  >;
}
...
const briefArtifact = {
  ...
  constraints: [
    { type: "hard", description: "Node.js 20+ runtime is required.", source: "platform" },
  ],
  non_goals: [
    { description: "Do not add external paid model APIs.", reason: "Core must stay API-key free." },
  ],
  ...
  verification_commands: [
    {
      ...
      working_directory: ".",
      evidence_roles: ["build", "quality-static", "quality-tests", "post-build"],
      evidence_kind: "tests",
    },
  ],
};
...
function expectDriftArtifactValid(): void {
  ...
  writeFileSync(
    join(workspaceRoot, "implementation.md"),
  ...
  expectSchemaValid(
    ...
    runDriftDetect(input, [], { workspaceRoot }),
  );
...
const invalidDesignArtifact = {
  ...
  iteration_history: [
    { iteration: 1, changes: "Initial draft", rationale: "Test required research timestamp" },
  ],
};
