...
(() => {
  ...
  function applyCheckpointDecision(run, body) {
    ...
    eventsByRun[run.id].push({
      seq: eventsByRun[run.id].length + 1,
      ts: run.updated_at,
  ...
  async function demoFetch(input, options = {}) {
    ...
    if (method === "GET" && eventsMatch) {
      const events = structuredClone(eventsByRun[decodeURIComponent(eventsMatch[1])] || []);
    ...
    return json({ error: { message: "Unsupported static-demo request." } }, 404);
  }
