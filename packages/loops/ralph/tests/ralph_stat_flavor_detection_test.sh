#!/usr/bin/env bash
# Regression coverage for Ralph's stat flavor detection contract.
...
run_case() {
  ...
  if [[ "$rc" -eq 0 ]]; then
    fail_case "gnu-stat-flavor-detection" "expected failure for out-of-scope modification, got success" "$tmpdir/out.log" "$tmpdir"
  fi
  if ! printf '%s' "$out" | grep -q 'modified files outside scope'; then
    fail_case "gnu-stat-flavor-detection" "expected scope violation message" "$tmpdir/out.log" "$tmpdir"
  fi
  if ! printf '%s' "$out" | grep -q 'src/outside.txt'; then
    fail_case "gnu-stat-flavor-detection" "expected offending path in output" "$tmpdir/out.log" "$tmpdir"
  fi

  cleanup_dir "$tmpdir"
  printf 'PASS [gnu-stat-flavor-detection]\n'
}

run_case
printf 'All stat flavor detection tests passed.\n'
