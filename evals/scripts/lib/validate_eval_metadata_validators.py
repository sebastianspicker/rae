...
def validate_benchmark_card(path: pathlib.Path) -> tuple[str, str]:
    ...
    if not isinstance(baselines, dict) or not baselines:
        raise ValueError(
            f"{path.relative_to(ROOT)} regression_policy.baseline_results must be a non-empty object"
        )
    ...
    if not set(required_splits).issubset(set(split_policy)):
        raise ValueError(
            f"{path.relative_to(ROOT)} release_gate.required_splits must be a subset of split_policy"
        )
    ...
    for claim_link in required_claim_links:
        if claim_link not in claim_links:
            raise ValueError(
                f"{path.relative_to(ROOT)} release_gate.required_claim_links must be included in claim_links"
            )
...
def validate_run_card(path: pathlib.Path, benchmarks: set[tuple[str, str]]) -> None:
    ...
    if evidence_type == "benchmark-run":
        ...
        missing_benchmark_fields = required_benchmark_fields - set(data)
        if missing_benchmark_fields:
            missing_str = ", ".join(sorted(missing_benchmark_fields))
            raise ValueError(
                f"{path.relative_to(ROOT)} is missing benchmark-run keys: {missing_str}"
            )
        ...
        if not is_within_directory(result_path, RESULTS_ROOT):
            raise ValueError(
                f"{path.relative_to(ROOT)} result_path must point under evals/results"
            )
    else:
        ...
        missing_observation_fields = required_observation_fields - set(data)
        if missing_observation_fields:
            missing_str = ", ".join(sorted(missing_observation_fields))
            raise ValueError(
                f"{path.relative_to(ROOT)} is missing observation keys: {missing_str}"
            )
        ...
        if evidence_type == "vendor-doc":
            ...
            if not isinstance(sources, list) or not sources:
                raise ValueError(
                    f"{path.relative_to(ROOT)} vendor-doc observation requires source_links"
                )
...
def validate_task_bundle(path: pathlib.Path) -> None:
    ...
    for task in tasks:
        ...
        if task.get("split") not in BENCHMARK_SPLITS:
            raise ValueError(
                f"{path.relative_to(ROOT)} task {task_id} has invalid split"
            )
        ...
        if workflow_verb is not None and workflow_verb not in WORKFLOW_VERBS:
            raise ValueError(
                f"{path.relative_to(ROOT)} task {task_id} has invalid workflow_verb"
            )
        if task.get("delegation_contract") is not None:
            validate_delegation_contract(
                task.get("delegation_contract"),
                f"{path.relative_to(ROOT)} task {task_id} delegation_contract",
            )
        claim_links = task.get("claim_links", [])
        if not isinstance(claim_links, list):
            raise ValueError(
                f"{path.relative_to(ROOT)} task {task_id} claim_links must be an array"
            )
        for claim_link in claim_links:
            if not isinstance(claim_link, str) or not path_exists(claim_link):
                raise ValueError(
...
def validate_benchmark_schema_patterns() -> None:
    ...
    for benchmark_path in iter_benchmark_paths():
        ...
        if not isinstance(task_specs_path, str) or not task_specs_pattern.match(
            ...
            raise ValueError(
                f"{benchmark_path.relative_to(ROOT)} task_specs_path does not match benchmark-card schema pattern"
            )
