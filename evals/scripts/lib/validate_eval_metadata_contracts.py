...
def validate_command_contract(data: object, label: str) -> None:
    ...
    require_non_empty_string(
        data.get("command"), f"{label}.command must be a non-empty string"
    )
...
def validate_delegation_contract(data: object, label: str) -> None:
    ...
    for entry in allowed_paths:
        require_non_empty_string(
            entry, f"{label}.allowed_paths entries must be strings"
        )
    ...
    require_non_empty_string(
        guard.get("rule"), f"{label}.guard.rule must be a non-empty string"
    )
...
def validate_verification_evidence(data: object, label: str) -> None:
    ...
    if required_types is not None:
        ...
        for entry in required_types:
            require_non_empty_string(
                entry, f"{label}.required_types entries must be strings"
            )
    ...
    for key in ("provided_types", "missing_types", "residual_gaps"):
        ...
        for entry in values:
            require_non_empty_string(
                entry, f"{label}.summary.{key} entries must be strings"
            )
    ...
    if task_statuses is not None:
        ...
        for index, entry in enumerate(task_statuses):
            ...
            if entry.get("status") not in EVIDENCE_SUMMARY_STATUSES:
                raise ValueError(
                    f"{label}.task_statuses[{index}].status has invalid value"
                )
            missing_types = entry.get("missing_types")
            if not isinstance(missing_types, list):
                raise ValueError(
                    f"{label}.task_statuses[{index}].missing_types must be an array"
                )
...
def validate_judge_config(path: pathlib.Path) -> None:
    ...
    if not isinstance(cases, list) or not cases:
        raise ValueError(
            f"{path.relative_to(ROOT)} calibration_cases must be non-empty"
        )
    for case in cases:
        if not isinstance(case, dict):
            raise ValueError(
                f"{path.relative_to(ROOT)} calibration case must be an object"
            )
...
def validate_baseline_result(path: pathlib.Path) -> None:
    ...
    if not isinstance(metrics, dict) or not metrics:
        raise ValueError(
            f"{path.relative_to(ROOT)} aggregate_metrics must be a non-empty object"
        )
