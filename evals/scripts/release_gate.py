...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate release-blocking gates for a benchmark run.")
...
def _ledger_issues(ledger_path: pathlib.Path, run_id: object) -> list[str]:
    return ["result ledger missing"] if not ledger_path.exists() else _ledger_registration_issues(ledger_path, run_id)
...
def main() -> int:
    ...
    issues = validate_run_card_contract(benchmark, run_card, run_card_path, regression_path, ledger_path, output)
    ...
    report = _build_gate_report(benchmark, run_card, issues, claim_links, ledger_path, regression_path)
